...
public class ContactUtil {
	...
	public static int getContactVersion(Context context, long id) {
		Cursor cursor = context.getContentResolver().query(
				...
				RawContacts.DELETED + "!=? AND " + RawContacts._ID + "=?",
				new String[] { String.valueOf(RAW_CONTACT_DELETE_FLAG),
	...
	public static List<ContactVersion> getAllContactVersions(Context context) {
		...
		Cursor cursor = context.getContentResolver().query(
				...
				RawContacts.DELETED + "!=" + RAW_CONTACT_DELETE_FLAG, null,
				null);

		if (cursor != null) {
			if (cursor.moveToFirst()) {
				final int IndexOfId = cursor.getColumnIndex(RawContacts._ID);
				final int indexOfVersion = cursor
						.getColumnIndex(RawContacts.VERSION);
	...
	public static List<GroupInfo> getAllGroups(Context context) {
		...
		if (cursorOfGroup.moveToFirst()) {
			...
			final int GROUP_ACCOUNT_TYPE = cursorOfGroup
					.getColumnIndex(Groups.ACCOUNT_TYPE);
			final int GROUP_ACCOUNT_NAME = cursorOfGroup
					.getColumnIndex(Groups.ACCOUNT_NAME);
			do {
				...
				gi.accountInfo.accountType = cursorOfGroup
						.getString(GROUP_ACCOUNT_TYPE);
				gi.accountInfo.accountName = cursorOfGroup
						.getString(GROUP_ACCOUNT_NAME);
	...
	public static int getSimCardState(Context context) {
		int count = 0;
		String where = RawContacts.ACCOUNT_NAME + "=?" + " and "
				+ RawContacts.ACCOUNT_TYPE + "=?";
		String whereargs1[] = new String[] { SIM1_ACCOUNT_NAME,
				SIM1_ACCOUNT_TYPE };
		String whereargs2[] = new String[] { SIM2_ACCOUNT_NAME,
				SIM2_ACCOUNT_TYPE };
		Cursor cursor = context.getContentResolver().query(
		...
		if (contact.shouldUpdatePhoto) {
			if (hasField(context, Photo.CONTENT_ITEM_TYPE, contact.id)) {
				if (contact.photo != null) {
					...
					final String[] projection = new String[] { Data._ID };
					final String selection = Data.CONTENT_TYPE + "=? and"
							+ Data.RAW_CONTACT_ID + "=?";
					...
					Cursor cursor = context.getContentResolver().query(
							...
							selectionArgs, null);
					if (cursor.moveToFirst()) {
						dataPhotoId = cursor.getLong(cursor
			...
			for (GroupInfo gi : contact.groupInfos) {
				ops.add(ContentProviderOperation
						...
						.withValue(Data.MIMETYPE,
								GroupMembership.CONTENT_ITEM_TYPE)
						.withValue(GroupMembership.GROUP_ROW_ID, gi.grId)
						.build());
			}
		}
		...
		if (contact.imInfos != null) {
			...
			for (ImInfo ir : contact.imInfos) {
				ops.add(ContentProviderOperation.newInsert(Data.CONTENT_URI)
						...
						.withValue(Im.CUSTOM_PROTOCOL, ir.customProtocol)
						.build());
			}
		}
	...
	public static Queue<Long> updateContact(Context context, Contact contact) {
				...
				if (contact.photo != null) {
					...
					Cursor cursor = context.getContentResolver().query(
							...
							new String[] { Photo.CONTENT_ITEM_TYPE,
									String.valueOf(contact.id) }, null);
			...
			if (gi.modifyFlag == ModifyTag.add) {
				...
				ops.add(ContentProviderOperation
						...
						.withSelection(Data._ID + "=?",
								new String[] { String.valueOf(gi.dataId) })
						.withValue(GroupMembership.GROUP_ROW_ID, gi.grId)
						.build());
			}
		}
		...
		for (PhoneInfo pr : contact.phoneInfos) {
			if (pr.modifyFlag == ModifyTag.add) {
				...
				ops.add(ContentProviderOperation
						.newUpdate(Data.CONTENT_URI)
						.withSelection(Data._ID + "=?",
								new String[] { String.valueOf(pr.id) })
						.withValue(Phone.NUMBER, pr.number)
						.withValue(Phone.TYPE, pr.type)
						.withValue(Phone.LABEL, pr.customName).build());
			}
		...
		for (EmailInfo er : contact.emailInfos) {
			if (er.modifyFlag == ModifyTag.add) {
				...
				ops.add(ContentProviderOperation
						.newUpdate(Data.CONTENT_URI)
						.withSelection(Data._ID + "=?",
								new String[] { String.valueOf(er.id) })
						.withValue(Email.ADDRESS, er.address)
						.withValue(Email.TYPE, er.type)
						.withValue(Email.LABEL, er.customName).build());
			}
		...
		for (OrgInfo or : contact.orgInfos) {
			if (or.modifyFlag == ModifyTag.add) {
				...
				ops.add(ContentProviderOperation
						.newUpdate(Data.CONTENT_URI)
						.withSelection(Data._ID + "=?",
								new String[] { String.valueOf(or.id) })
						.withValue(Organization.COMPANY, or.company)
						.withValue(Organization.TYPE, or.type)
						.withValue(Organization.LABEL, or.customName).build());
			}
		...
		for (AddressInfo ar : contact.addressInfos) {
			if (ar.modifyFlag == ModifyTag.add) {
				...
				ops.add(ContentProviderOperation
						.newUpdate(Data.CONTENT_URI)
						.withSelection(Data._ID + "=?", new String[] { String.valueOf(ar.id) })
						.withValue(StructuredPostal.FORMATTED_ADDRESS, ar.address)
						.withValue(StructuredPostal.COUNTRY, ar.country)
						.withValue(StructuredPostal.CITY, ar.city)
						.withValue(StructuredPostal.STREET, ar.street)
		...
		for (ImInfo ir : contact.imInfos) {
			if (ir.modifyFlag == ModifyTag.add) {
				...
				ops.add(ContentProviderOperation
						.newUpdate(Data.CONTENT_URI)
						.withSelection(Data._ID + "=?",
								new String[] { String.valueOf(ir.id) })
						.withValue(Im.DATA, ir.account)
						.withValue(Im.PROTOCOL, ir.protocol)
						.withValue(Im.CUSTOM_PROTOCOL, ir.customProtocol)
						.build());
	...
	public static long addContact(Context context, Contact contact) {
		...
		ops.add(ContentProviderOperation
				...
				.withValue(ContactsContract.RawContacts.ACCOUNT_TYPE,
						contact.accountInfo.accountType)
				.withValue(ContactsContract.RawContacts.ACCOUNT_NAME,
						contact.accountInfo.accountName)
		...
		if (contact.orgInfos.size() > 0) {
			for (Iterator<OrgInfo> iter = contact.orgInfos.iterator(); iter
					.hasNext();) {
		...
		if (contact.phoneInfos.size() > 0) {
			for (Iterator<PhoneInfo> iter = contact.phoneInfos.iterator(); iter
					.hasNext();) {
		...
		if (contact.emailInfos.size() > 0) {
			for (Iterator<EmailInfo> iter = contact.emailInfos.iterator(); iter
					.hasNext();) {
		...
		if (contact.addressInfos.size() > 0) {
			for (Iterator<AddressInfo> iter = contact.addressInfos.iterator(); iter
					.hasNext();) {
		...
		if (contact.imInfos.size() > 0) {
			for (Iterator<ImInfo> iter = contact.imInfos.iterator(); iter
					.hasNext();) {
				ImInfo ir = iter.next();
				ops.add(ContentProviderOperation
						.newInsert(ContactsContract.Data.CONTENT_URI)
	 ...
	 * @param context
	 */
	public static List<GroupInfo> getGroupByAccount(Context context,
			AccountInfo ai) {
		if (ai == null) {
			ai = new AccountInfo();
			ai.accountName = DEFAULT_ACCOUNT_NAME;
		...
		String whereArgs[] = { ai.accountName, ai.accountType };
		...
		try {
			if (cursor.moveToFirst()) {
				...
				final int GROUP_ACCOUNT_NAME = cursor
						.getColumnIndex(Groups.ACCOUNT_NAME);
				final int GROUP_ACCOUNT_TYPE = cursor
						.getColumnIndex(Groups.ACCOUNT_TYPE);
				do {
					GroupInfo gi = new GroupInfo();
					gi.grId = cursor.getLong(GROUP_ID);
					gi.name = cursor.getString(GROUP_TITLE);
					gi.accountInfo.accountType = cursor
							.getString(GROUP_ACCOUNT_NAME);
					gi.accountInfo.accountName = cursor
							.getString(GROUP_ACCOUNT_TYPE);
					ls.add(gi);
				} while (cursor.moveToNext());
			}
	...
	public static boolean isContactInGroup(Context context, long rawContactId) {
		...
		String where = Data.RAW_CONTACT_ID + "=?" + " and " + Data.MIMETYPE
				+ " = ?";
		String whereArgs[] = { String.valueOf(rawContactId),
				GroupMembership.CONTENT_ITEM_TYPE };
	...
	public static boolean updateGroup(Context context, GroupInfo groupInfo) {
		...
		int rows = context.getContentResolver().update(Groups.CONTENT_URI, cv,
				Groups._ID + "=?",
				new String[] { String.valueOf(groupInfo.grId) });
		return rows > 0;
	}
	 ...
	 * @return
	 */
	public static boolean hasField(Context context, String mimeType,
			long rawContactId) {
		String[] projection = { Data._ID };
		String where = Data.MIMETYPE + "=? and " + Data.RAW_CONTACT_ID + "=?";
		String[] whereArgs = { mimeType, String.valueOf(rawContactId) };
	...
	public static Contact getContactById(Context context, long rawContactId) {
		...
		Collection<Contact> contacts = getContactsFast(context, selection,
				selectionArgs);
	...
	private static Collection<Contact> getContactsFast(Context context,
			String selection, String[] selectionArgs) {
		Map<Long, Contact> contactsMap = new HashMap<Long, Contact>();

		{
			String[] projection = { RawContacts._ID, RawContacts.ACCOUNT_NAME,
					RawContacts.ACCOUNT_TYPE, RawContacts.VERSION };

			Cursor cursor = context.getContentResolver().query(
					RawContacts.CONTENT_URI, projection, selection,
					selectionArgs, null);

			if (cursor != null) {
				try {
					if (cursor.moveToFirst()) {
						final int idxForId = cursor
								.getColumnIndex(RawContacts._ID);
						final int idxForAccountName = cursor
								.getColumnIndex(RawContacts.ACCOUNT_NAME);
						final int idxForAccountType = cursor
								.getColumnIndex(RawContacts.ACCOUNT_TYPE);
						final int idxForVersion = cursor
								.getColumnIndex(RawContacts.VERSION);

						do {
							Contact contact = new Contact();
							contact.id = cursor.getLong(idxForId);
							contact.accountInfo.accountName = cursor
									.getString(idxForAccountName);
							contact.accountInfo.accountType = cursor
									.getString(idxForAccountType);
		...
		{
			String[] projection = { RawContactsEntity._ID,
			...
			Cursor cursor = context.getContentResolver().query(
					RawContactsEntity.CONTENT_URI, projection, selection,
					selectionArgs, RawContactsEntity._ID);
			if (cursor != null) {
				try {
					if (cursor.moveToFirst()) {
						final int idxForRawContactId = cursor
								.getColumnIndex(RawContactsEntity._ID);
						final int idxForDataId = cursor
								.getColumnIndex(RawContactsEntity.DATA_ID);
						final int idxForMimeType = cursor
								.getColumnIndex(RawContactsEntity.MIMETYPE);
						final int idxForData1 = cursor
								.getColumnIndex(RawContactsEntity.DATA1);
						final int idxForData2 = cursor
								.getColumnIndex(RawContactsEntity.DATA2);
						final int idxForData3 = cursor
								.getColumnIndex(RawContactsEntity.DATA3);
						final int idxForData4 = cursor
								.getColumnIndex(RawContactsEntity.DATA4);
						final int idxForData5 = cursor
								.getColumnIndex(RawContactsEntity.DATA5);
						final int idxForData6 = cursor
								.getColumnIndex(RawContactsEntity.DATA6);
						final int idxForData7 = cursor
								.getColumnIndex(RawContactsEntity.DATA7);
						final int idxForData8 = cursor
								.getColumnIndex(RawContactsEntity.DATA8);
						final int idxForData9 = cursor
								.getColumnIndex(RawContactsEntity.DATA9);
						final int idxForData10 = cursor
								.getColumnIndex(RawContactsEntity.DATA10);
						final int idxForData11 = cursor
								.getColumnIndex(RawContactsEntity.DATA11);
						final int idxForData12 = cursor
								.getColumnIndex(RawContactsEntity.DATA12);
						final int idxForData13 = cursor
								.getColumnIndex(RawContactsEntity.DATA13);
						final int idxForData14 = cursor
								.getColumnIndex(RawContactsEntity.DATA14);
						final int idxForData15 = cursor
								.getColumnIndex(RawContactsEntity.DATA15);

						do {
							final long rawContactId = cursor
									.getLong(idxForRawContactId);
							Contact contact = contactsMap.get(rawContactId);
							if (contact != null) {
								String mimeType = cursor
										.getString(idxForMimeType);
								...
								if (mimeType
									...
									contact.name = cursor
											.getString(idxForData1);
								} else if (mimeType
										.equals(Nickname.CONTENT_ITEM_TYPE)) {
									contact.nickname = cursor
											.getString(idxForData1);
								} else if (mimeType
										.equals(Photo.CONTENT_ITEM_TYPE)) {
									contact.photo = cursor
											.getBlob(idxForData15);
								} else if (mimeType
										.equals(GroupMembership.CONTENT_ITEM_TYPE)) {
									GroupInfo groupInfo = new GroupInfo();

									groupInfo.grId = cursor
											.getLong(idxForData1);
									groupInfo.dataId = cursor
											.getLong(idxForDataId);
									...
									phoneInfo.number = cursor
											.getString(idxForData1); // number
									phoneInfo.type = cursor.getInt(idxForData2); // type
									if (phoneInfo.type == Phone.TYPE_CUSTOM) {
										phoneInfo.customName = cursor
												.getString(idxForData3); // label
									}
									...
									emailInfo.address = cursor
											.getString(idxForData1); // address
									emailInfo.type = cursor.getInt(idxForData2); // type
									if (emailInfo.type == Email.TYPE_CUSTOM) {
										emailInfo.customName = cursor
												.getString(idxForData3); // label
									}
									...
									imInfo.account = cursor
											.getString(idxForData1); // data
									imInfo.protocol = cursor
											.getInt(idxForData5); // protocol
									if (imInfo.protocol == Im.PROTOCOL_CUSTOM) {
										imInfo.customProtocol = cursor
												.getString(idxForData6); // custom
																			// protocol
									}
									...
									orgInfo.company = cursor
											.getString(idxForData1); // company
									...
									addressInfo.id = cursor
											.getLong(idxForDataId);
									addressInfo.address = cursor
											.getString(idxForData1); // formated
																		// address
									addressInfo.type = cursor
											.getInt(idxForData2); // type
									addressInfo.street = cursor
											.getString(idxForData4); // street
									addressInfo.city = cursor
											.getString(idxForData7); // city
									addressInfo.region = cursor
											.getString(idxForData8); // region
									addressInfo.postcode = cursor
											.getString(idxForData9); // postcode
									addressInfo.country = cursor
											.getString(idxForData10);// country

									if (addressInfo.type == StructuredPostal.TYPE_CUSTOM) {
										addressInfo.customName = cursor
												.getString(idxForData3); // label
									}
