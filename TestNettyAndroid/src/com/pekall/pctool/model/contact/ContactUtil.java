...
public class ContactUtil {
	...
	public static Contact getContactByRawId(Context context, long rawId) {
		...
		{
			Cursor cursorOfContact = resolver.query(rawContactUri,
					new String[] { RawContacts.CONTACT_ID }, RawContacts._ID
							+ "=?", new String[] { rawContactId }, null);
			while (cursorOfContact.moveToNext()) {
				long contactId = cursorOfContact.getLong(cursorOfContact
...

		{
			Cursor cursorOfAccount = resolver.query(rawContactUri,
					new String[] { RawContacts.ACCOUNT_NAME,
							RawContacts.ACCOUNT_TYPE }, RawContacts._ID + "=?",
					new String[] { rawContactId }, null);
...

		{
			...
			while (cursorOfGroup.moveToNext()) {
				...
				Cursor cursorOfGroupInfo = context.getContentResolver().query(
						Groups.CONTENT_URI,
						new String[] { Groups.TITLE, Groups.NOTES },
						Groups._ID + " = ?  ",
						new String[] { String.valueOf(gr.grId) }, null);
...

		{
			...
			Cursor cursorOfNickName = context.getContentResolver()
					.query(Data.CONTENT_URI,
							...
							Data.RAW_CONTACT_ID + " = ? and " + Data.MIMETYPE
									+ " = ? ",
							new String[] { rawContactId, Nickname.TYPE }, null);
		...
		{
			// phone Number
			Cursor cursorOfPhone = context.getContentResolver()
					.query(Data.CONTENT_URI,
							new String[] { Data._ID, Phone.NUMBER,// ----data1
		...
		// email
		{
			Cursor cursorOfEmail = context.getContentResolver()
					.query(Data.CONTENT_URI,
							new String[] { Data._ID, Email.ADDRESS,// data1
		...
		// im
		{
			Cursor cursorOfIm = context
					.getContentResolver()
					.query(Data.CONTENT_URI, // 查询data表
							new String[] { Data._ID, Im.PROTOCOL, // --->data5(type)
									Data.DATA1, Data.DATA3
							// IM IM.type means home work and so on
							},
							Data.RAW_CONTACT_ID + " = ? and " + Data.MIMETYPE
									+ " = ? ",
							new String[] { rawContactId, Im.CONTENT_ITEM_TYPE, },
							null);
		...
		// address
		{
			Cursor cursorOfAddress = context
					.getContentResolver()
					.query(Data.CONTENT_URI,
							new String[] { Data._ID,
							...
							Data.RAW_CONTACT_ID + " = ? and " + Data.MIMETYPE
									+ " = ? ",
							new String[] { rawContactId,
									StructuredPostal.CONTENT_ITEM_TYPE, }, null);
		...
		// organization
		{
			Cursor cursorOfOrganization = context.getContentResolver()
					.query(Data.CONTENT_URI,
	...
	public static int getSimCardState(Context context) {
		...
		Cursor cursor = context.getContentResolver().query(
				...
				new String[] { SIM1_ACCOUNT_NAME, SIM1_ACCOUNT_TYPE }, null);
		if (cursor.getCount() > 0)
			count++;
		cursor.close();
		cursor = context.getContentResolver().query(
				ContactsContract.RawContacts.CONTENT_URI, null, where,
				new String[] { SIM2_ACCOUNT_NAME, SIM2_ACCOUNT_TYPE }, null);
		if (cursor.getCount() > 0)
			count += 2;
		cursor.close();
		return count;
		...
		for (AddressInfo ar : contact.addressInfos) {
			if (ar.modifyFlag == ModifyTag.add) {
				...
				ops.add(ContentProviderOperation
						.newUpdate(ContactsContract.Data.CONTENT_URI)
						.withSelection(Data._ID + "=?",
								new String[] { String.valueOf(ar.id) })
						.withValue(StructuredPostal.FORMATTED_ADDRESS,
								ar.address)
						.withValue(StructuredPostal.COUNTRY, ar.country)
						.withValue(StructuredPostal.CITY, ar.city)
	...
	public static List<GroupInfo> queryGroup(Context context, AccountInfo ai) {
		...
		Cursor mr = context.getContentResolver().query(uri, null, where,
				new String[] { ai.accountName, ai.accountType }, null);
		List<GroupInfo> ls = new ArrayList<GroupInfo>();
		while (mr.moveToNext()) {
			GroupInfo gi = new GroupInfo();
			gi.grId = mr
					.getLong(mr.getColumnIndex(ContactsContract.Groups._ID));
			gi.name = mr.getString(mr
					.getColumnIndex(ContactsContract.Groups.TITLE));
			gi.accountInfo.accountType = mr.getString(mr
					.getColumnIndex(ContactsContract.Groups.ACCOUNT_TYPE));
			gi.accountInfo.accountName = mr.getString(mr
	...
	public static boolean updateGroup(Context context, GroupInfo gi) {
		...
		int rows = context.getContentResolver().update(groupUri, cv,
				Groups._ID + "=?", new String[] { String.valueOf(gi.grId) });
		if (rows > 0)
			return true;
		return false;
	}
	...
	public static List<Contact> getContactsByGroup(Context context, GroupInfo gi) {
		...
		String RAW_CONTACTS_WHERE = ContactsContract.CommonDataKinds.GroupMembership.GROUP_ROW_ID
				...
				+ "'";
		Cursor mr = context.getContentResolver().query(
				ContactsContract.Data.CONTENT_URI, RAW_PROJECTION,
				RAW_CONTACTS_WHERE, new String[] { String.valueOf(gi.grId) },
				null);
		while (mr.moveToNext()) {
	...
	public static List<Contact> getFavoriteContacts(Context context) {
		...
		Cursor cursor = context.getContentResolver().query(
				ContactsContract.RawContacts.CONTENT_URI,
				new String[] { RawContacts._ID }, where,
				new String[] { String.valueOf(FAVORITE_CONTACT_FLAG) }, null);
		while (cursor.moveToNext()) {
			long rawContactId = cursor.getLong(cursor
	...
	public static boolean hasField(String filed, Context context, long rawId) {
		...
		if (filed.equals("name"))
			whereargs = new String[] { StructuredName.CONTENT_ITEM_TYPE,
					String.valueOf(rawId) };
		else if (filed.equals("nickname"))
			whereargs = new String[] { Nickname.CONTENT_ITEM_TYPE,
					String.valueOf(rawId) };
		else if (filed.equals("photo"))
			whereargs = new String[] { Photo.CONTENT_ITEM_TYPE,
					String.valueOf(rawId) };
		Cursor cursor = context.getContentResolver().query(dataUri,
				new String[] { Data._ID }, where, whereargs, null);
		if (cursor.getCount() > 0) {
			cursor.close();
			return true;
		}
